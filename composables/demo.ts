...
import { DOMURL, isDev } from '@/lib/utils'

import type { DrawingMode } from 'drauu'
import type { HandwrittenResult } from '~~/lib/types'
...
export function useHandWritten(svgEl: Ref<SVGSVGElement | null>) {
  ...
  const { data, loading, error, post, cancel } = useApi<HandwrittenResult>(`${config.functionsUrl}/handwritten`, {
    headers: {
